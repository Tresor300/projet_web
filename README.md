...
1. **Déclenchement** — un push ou une pull request lance la chaîne.
2. **Analyse** — l'image et le code sont scannés, un inventaire des dépendances (SBOM) est généré.
3. **Triage déterministe** — chaque vulnérabilité est notée sur des critères vérifiables : probabilité d'exploitation (EPSS), présence au catalogue des failles activement exploitées (CISA KEV), atteignabilité réelle du composant vulnérable, et présence dans l'image d'exécution plutôt qu'à l'étape de build.
4. **Enrichissement** — un modèle de langage explique les failles retenues et rédige les pull requests de correctif.
5. **Blocage** — le cluster Kubernetes refuse toute image non signée ou porteuse d'une faille critique.
6. **Suivi** — un tableau de bord expose la couverture, le délai moyen de remédiation et le taux de conformité.
...
Chaque étape est documentée dans [`journal.md`](journal.md) : le problème résolu, ce qui casserait sans elle, et les points encore ouverts.
